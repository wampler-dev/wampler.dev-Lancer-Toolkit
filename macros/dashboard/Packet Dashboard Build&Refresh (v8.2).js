// Packet Dashboard — Build/Refresh (v8.2) [Meta-aware]
// (Full macro; safe replacement)
...
function findMissionMetaFolder(prefix, mission, missionName){
  const rootName = `${prefix} M${mission} — ${missionName}`;
  const root = game.folders.contents.find(f => f.type==="JournalEntry" && f.name===rootName) ?? null;
  if (!root) return null;
...
async function promptDashboard(){
  return new Promise(resolve=>{
    new Dialog({
      title:"Packet Dashboard — Build/Refresh (v8.2) [Meta-aware]",
      content:`
      <form>
        ...
        <div class="form-group">
          ...
          Show Scene links in Upcoming/Completed lists too</label>
        </div>

        <hr>
        <h3>Macro Links</h3>
        <div class="form-group"><label>Open/Refresh Dashboard</label><input name="dashSelf" value="Packet Dashboard — Build/Refresh (v8.2)"/></div>
...
function uuidLink(j){ return `@UUID[JournalEntry.${j.id}]{${j.name}}`; }
...
function macroLink(name, label){ return name ? `@Macro[${name}]{${label}}` : ""; }
...
const metaFolder = findMissionMetaFolder(cfg.prefix, cfg.mission, cfg.missionName);
...
} else if (metaFolder && dashboard.folder?.id !== metaFolder.id) {
  await dashboard.update({ folder: metaFolder.id });
}
...
const metaNote = metaFolder
  ? `<p class="notes"><em>Meta folder:</em> Meta (reused)</p>`
  : `<p class="notes" style="color:#b00;"><em>Meta folder not found</em> — dashboard will be created at root until you run the v9 generator.</p>`;
